...
class Client {
  ...
  async get(params) {
    ...
    return await this.fetch(url, this.httpOptions);
  }
  ...
  async getRepository() {
    ...
    return await this.fetch(url.toString(), this.httpOptions);
  }
  async getRefs() {
  ...
  async getTags() {
    try {
      ...
      return await this.fetch(tagsForm.action, this.httpOptions);
    } catch (e) {
      const res = await this.getRepository();
      return res.tags;
    }
